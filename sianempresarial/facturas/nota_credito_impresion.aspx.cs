...
public partial class facturas_nota_credito_impresion : BasePagePopUp
{
    ...
    private void Obtener_Datos()
    {
        ...
        strQuery = "SELECT P.nombre, F.detalle as detalle " +
                    ",P.usar_detalle" +
                    " FROM notas_credito_prod F" +
                    " INNER JOIN productos P" +
                    " ON F.producto_ID = P.ID " +
                    " WHERE F.nota_ID = " + strNotaID +
    ...
    private void Crear_Archivo(int intTotalPaginas)
    {
        ...
        else
        {
            ...
            if (File.Exists(Server.MapPath("../xml_facturas" + HttpContext.Current.Request.ApplicationPath + "/" + strNota)))
            {
                Response.Redirect("facturas_correo.aspx?t=2&ID=" + strNotaID);
            }
    ...
    class MyPageEvents : PdfPageEventHelper
    {
        ...
        public override void OnStartPage(PdfWriter writer, Document document)
        {
            ...
            // Datos de la nota
            texto = new Paragraph("NOTA DE CRÉDITO, FOLIO: ", ftHeaderB);

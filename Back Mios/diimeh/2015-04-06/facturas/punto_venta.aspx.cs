...
public partial class facturas_punto_venta : BasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        this.txtCambiarCantidad.Attributes["onkeypress"] = "javascript:return isNumber(event, this);";
        ...
        if (!IsPostBack)
        {
            ...
            this.rdIVA.Items[0].Selected = true;
            ...
            this.hdInvAlmacen.Value = ConfigurationManager.AppSettings["invAlmacen"].ToString();
            ...
            this.hdVentaVendedor.Value = ConfigurationManager.AppSettings["vendedorPuntoVenta"].ToString();
    ...
    private DataTable ObtenerLotes(string strID)
    {
        ...
        string strQuery = "SELECT existencia, lote, fecha_caducidad" +
                          " FROM inventario " +
                          " WHERE producto_ID = " + strID +
                          "   AND subalmacen_ID = 100 " +
                          " ORDER BY existencia desc, fecha_caducidad, lote";
        try
        {
            objDataResult = CComunDB.CCommun.Ejecutar_SP(strQuery);
        }
    ...
    private void Mostrar_Datos()
    {
        ...
        else
        {
            ...
            string strQuery = "SELECT sucursal_ID, concat(negocio, ' - ', sucursal) as nombre, " +
                    " nota, nota_suf, fecha, F.status, F.contado, comentarios, dirigidoa, " +
                    " F.descuento, F.descuento2, F.iva, F.lista_precios_ID, vendedorID, " +
    ...
    {
        this.btnFecha.Enabled = sw_estado;
    ...
    private bool Crear_Nota(string strNota, string strNota_Suf,
                            ...
                            string strComentarios, string strDirigidoA,
                            int intListaPreciosID, int intVendedorID)
    {
        DataSet objDataResult = new DataSet();
        ...
        objNota.dirigidoa = strDirigidoA;
        objNota.status = 8;
    ...
    protected void btnVerificacionContinuar_Click(object sender, EventArgs e)
    {
        ...
        CRutinas.Enviar_Correo("6",
                               ...
                               "Remisión: " + this.hdID.Value + "<br />" +
                               "Cliente: Punto de Venta" + "<br />" +
                               "Razón: Remisión creada sin pedido previo" + "<br /><br />" +
    ...
    protected void btnAgregarProd_Click(object sender, ImageClickEventArgs e)
    {
            ...
            decimal.TryParse(this.txtPrecioUnitario.Text.Trim(), out dcmPrecioUnitario))
        {
            ...
            string strMensaje = CComunDB.CCommun.Validar_Ultimo_Precio_Compra(int.Parse(this.hdProductoID.Value), dcmPrecioUnitario);
            if (!string.IsNullOrEmpty(strMensaje))
            {
    ...
    protected void btnProdVentaContinuar_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            StringWriter swTexto = new StringWriter(strTexto);
            HtmlTextWriter twTexto = new HtmlTextWriter(swTexto);
            this.pnlProdVenta.RenderControl(twTexto);
        }
        catch
        {
            strTexto = new StringBuilder();
        ...
        CRutinas.Enviar_Correo("6",
                               ...
                               "Remisión: " + this.hdID.Value + "<br />" +
                               "Cliente: Punto de Venta" + "<br />" +
                               "Razón: Precio de venta del producto es menor al último precio de compra" + "<br /><br />" +
                               strTexto.ToString().Replace("display:none;", ""));
    ...
    private bool Validar_Lote()
    {
        ...
        if (gvLote.Rows.Count > 0)
        {
            if (gvLote.Rows.Count == 1)
            {
                ...
                {
                    if (int.Parse(((HiddenField)gvLote.Rows[0].FindControl("hdCantidadLote")).Value) >=
                        int.Parse(this.txtCantidad.Text.Trim()))
                    {
            ...
            {
                ((TextBox)gvRow.FindControl("txtCantidadLote")).Attributes["onkeypress"] = "javascript:return isNumber(event);";
                                ...
                                out string strMensaje)
    {
        ...
        if (this.hdInventarios.Value.Equals("1"))
        {
            foreach (GridViewRow gvRow in this.gvLote.Rows)
            {
                if (int.Parse(((TextBox)gvRow.FindControl("txtCantidadLote")).Text.Trim()) > 0)
                {
    ...
    protected void gvProductos_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (e.Row.RowType == DataControlRowType.Footer)
            {
                ...
                if (this.hdHonorarios.Value.Equals("1"))
                {
                    dcmSubtotal = Math.Round(dcmCostoDescuento + dcmIVA, 2);
                    this.hdSubtotal.Value = dcmSubtotal.ToString();
                    strLeyenda.Append("<br />Subtotal:");
                    strValores.Append("<br />" + dcmSubtotal.ToString("c"));

                    dcmISRRet = Math.Round(dcmCostoDescuento * decimal.Parse(this.hdPorcISRRet.Value) / 100, 2);
                    this.hdISRRet.Value = dcmISRRet.ToString();
                    strLeyenda.Append("<br />ISR Retenido " + this.hdPorcISRRet.Value + "%:");
                    strValores.Append("<br />" + dcmISRRet.ToString("c"));

                    dcmIVARet = Math.Round(dcmCostoDescuento * decimal.Parse(this.hdPorcIVARet.Value) / 100, 2);
                    this.hdIVARet.Value = dcmIVARet.ToString();
                    strLeyenda.Append("<br />IVA Retenido " + this.hdPorcIVARet.Value + "%:");
                    strValores.Append("<br />" + dcmIVARet.ToString("c"));

                    dcmTotal = Math.Round(dcmSubtotal - dcmISRRet - dcmIVARet, 2);
        ...
        if (this.dlEstatus.SelectedValue.Equals("8"))
        {
            ...
            if (e.CommandName == "Borrar")
            {
                ...
                if (e.CommandName == "Modificar")
                {
                    ...
                    this.txtCambiarUnitario.Text = this.gvProductos.Rows[index].Cells[4].Text.Replace("$", "").Replace(",", "");
    ...
    protected void btnCancelarContinuar_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            StringWriter swTexto = new StringWriter(strTexto);
            HtmlTextWriter twTexto = new HtmlTextWriter(swTexto);
            this.pnlMessageCancelar.RenderControl(twTexto);
        }
        catch
        {
            strTexto = new StringBuilder();
        ...
        CRutinas.Enviar_Correo("6",
                               ...
                               "Remisión: " + this.hdID.Value + "<br />" +
                               "Cliente: Punto de Venta" + "<br />" +
                               "Razón: Remisión cancelada" + "<br /><br />" +
                               strTexto.ToString().Replace("display:none;", ""));
    }
    ...
    protected void btnCambiarContinuar_Click(object sender, EventArgs e)
    {
        ...
        if (dcmCantidad > 0 && dcmPrecio > 0)
        {
            ...
            string strMensaje = CComunDB.CCommun.Validar_Ultimo_Precio_Compra(int.Parse(strID_Consecutivo[0]), dcmPrecio);
            if (!string.IsNullOrEmpty(strMensaje))
            {
    ...
    protected void btnFacturarContinuar_Click(object sender, EventArgs e)
    {
        ...
        strQuery = "INSERT INTO pago_facturas (pagoID, facturaID, monto_pago, notaID)" +
                   " SELECT pagoID, " + intFacturaID +
                   " , monto_pago " +
                   " , " + this.hdID.Value +
                   " FROM pago_notas WHERE notaID = " + this.hdID.Value;
        try
        {
            CComunDB.CCommun.Ejecutar_SP(strQuery);
        }
        ...
        strQuery = "DELETE FROM pago_notas WHERE notaID = " + this.hdID.Value;
        try
        {
            CComunDB.CCommun.Ejecutar_SP(strQuery);
        }
        ...
        ((master_MasterPage)Page.Master).MostrarMensajeError("Nota ha sido facturada, folio: " + intFacturaID.ToString());
    ...
    private bool Validar_Inventario()
    {
        ...
        foreach (DataRow objRowResult in objDataResult.Tables[0].Rows)
        {
            if (decimal.Parse(objRowResult["cantidad"].ToString()) >
                CInventarios.Obtener_Existencia(objRowResult["productoID"].ToString(), "100",
                                               objRowResult["lote"].ToString()))
            {
                ((master_MasterPage)Page.Master).MostrarMensajeError("No hay suficiente cantidad en el inventario para el producto " +
                                    ...
                                    ", lote: " + objRowResult["lote"].ToString());
                return false;
            }
        }
    ...
    {
        DataSet objDataResult = new DataSet();
        string strQuery = "SELECT producto_ID, cantidad, costo_unitario, costo" +
                          " FROM notas_prod" +
                          " WHERE nota_ID = " + this.hdID.Value;
        ...
        foreach (DataRow objRowResult in objDataResult.Tables[0].Rows)
        {
            ...
            objProd_Datos.dcmNota_costo = (decimal)objRowResult["costo_unitario"];
            objProd_Datos.dcmNota_total = (decimal)objRowResult["costo"];
        ...
        foreach (DataRow objRowResult in objDataResult.Tables[0].Rows)
        {
            CInventarios.Restar(objRowResult["productoID"].ToString(), "100",
                                objRowResult["lote"].ToString(),
    ...
    private void Agregar_Producto_Fact(int intFacturaID, int intProductoID,
                                       decimal dcmCantidad, decimal dcmCosto_unitario,
                                       decimal dcmCosto, bool esExento,
                                       string strConsecutivo, bool usarSal, bool usarCveGob)
    {
        DataSet objDataResult = new DataSet();
        ...
        strQuery = "INSERT INTO facturas_liq_prod (factura_ID, " +
                ...
                "costo, consecutivo, inventariado, usar_sal, usar_cve_gob) VALUES (" +
    ...
    protected void btnImprimir_Click(object sender, ImageClickEventArgs e)
    {
        ...
        if (this.hdVentaVendedor.Value.Equals("5") ||
            this.hdVentaVendedor.Value.Equals("6"))
            strPopUP = "mostrarPopUp('nota_impresion_especial.aspx?notID=" +
                                        this.hdID.Value + "')";
        else
            if (this.gvProductos.Rows.Count <= 15)
                strPopUP = "mostrarPopUp('nota_impresion_especial.aspx?notID=" +
                                        this.hdID.Value + "')";
            else
                strPopUP = "mostrarPopUp('nota_impresion.aspx?notID=" +
                                            this.hdID.Value + "')";
    ...
    protected void btnLimiteCRContinuar_Click(object sender, EventArgs e)
    {
        ...
        CRutinas.Enviar_Correo("6",
                               ...
                               "Remisión: " + this.hdID.Value + "<br />" +
                               "Cliente: Punto de Venta" + "<br />" +
                               "Razón: Límite de crédito del cliente ha sido alcanzado" + "<br /><br />" +
                               strTexto.ToString().Replace("display:none;", ""));
    }
    ...
    private int Generar_Pedido()
    {
        ...
        strQuery = "INSERT INTO orden_compra (sucursalID, orden_compra, lista_preciosID, " +
                   ...
                   "porc_iva, iva, total, orden_compra_total, " +
                   "comentarios, creadoPorID, creadoPorFecha, modificadoPorID, modificadoPorFecha) VALUES (" +
        ...
        strQuery = "INSERT INTO orden_compra_productos (orden_compraID, " +
                  ...
                  "validado, faltante, consecutivo, usar_sal) " +
                  " SELECT " + intPedidoID +
                  ", producto_ID" +
                  ", exento" +
                  ", cantidad" +

...
public partial class facturas_factura_preimpresion : BasePagePopUp
{
    ...
    {
        strOrdenCompra = strProveedor = strFact = strReferencia = string.Empty;
        if (Request.QueryString["factID"] != null)
        {
            DataSet objDataResult = new DataSet();
            string strQuery = "SELECT F.factura, F.orden_compra, F.comentarios, F.referencia, E.proveedor " +
            ...
            if (objDataResult.Tables[0].Rows.Count > 0)
            {
                ...
                this.lblMessage.Text = "Factura no existe";
        }
        else
            this.lblMessage.Text = "Factura no existe";
    }
    ...
    private void Generar_Factura()
    {
        ...
        {
            this.lblMessage.Text = "Factura no tiene productos";
            return;
        }
    ...
    private void Obtener_Datos()
    {
        ...
        string strSucID = objFacturaDB.sucursal_ID.ToString();
        decimal dcmTotal = objFacturaDB.total;
        decimal dcmPorISR_Ret = objFacturaDB.isr_ret;
        decimal dcmISR_Ret = objFacturaDB.monto_isr_ret;
        decimal dcmPorIVA_Ret = objFacturaDB.iva_ret;
        decimal dcmIVA_Ret = objFacturaDB.monto_iva_ret;
        decimal dcmTotalRetenidos = dcmISR_Ret + dcmIVA_Ret;
        decimal dcmPorIva = objFacturaDB.iva;
        decimal dcmIva = objFacturaDB.monto_iva;
        decimal dcmPorcDescuento = objFacturaDB.descuento;
        decimal dcmPorcDescuento2 = objFacturaDB.descuento2;
        decimal dcmSubtotal =  objFacturaDB.monto_subtotal;
    ...
    private int Crear_Archivo_Temporal()
    {
        ...
        foreach (CProducto objProducto in objFactura.LstProductos)
        {
            ...
            texto = new Paragraph((objProducto.SwKit ? string.Empty : objProducto.AmtValorUnitario.ToString("0.##")), ftProductos);
            celda = new PdfPCell(texto);
            celda.HorizontalAlignment = Element.ALIGN_RIGHT;
            celda.Border = Rectangle.NO_BORDER;
            tblLinea.AddCell(celda);

            texto = new Paragraph((objProducto.SwKit ? string.Empty : objProducto.AmtImporte.ToString("0.##")), ftProductos);
            celda = new PdfPCell(texto);
            celda.HorizontalAlignment = Element.ALIGN_RIGHT;
            celda.Border = Rectangle.NO_BORDER;
            tblLinea.AddCell(celda);
    ...
    class MyPageEvents : PdfPageEventHelper
    {
        ...
        public override void OnStartPage(PdfWriter writer, Document document)
        {
            ...
            ancho_columnas[0] = 190;
            ancho_columnas[1] = tblHeaderA.TotalWidth - 380;
            ancho_columnas[2] = 190;
